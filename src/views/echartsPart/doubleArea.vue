<template>
  <div class="doubleArea" id="doubleArea"></div>
</template>
...
export default {
  ...
  methods: {
      ...
      switch (type) {
        case "x": // 设置横坐标 时间数据
          for (let i = 0; i < this.selectRangeDate.length; i++) {
            arr.push(
              this.selectRangeDate[i][0] +
                "." +
                this.selectRangeDate[i][1] +
                "." +
                this.selectRangeDate[i][2]
            );
          }
          break;
        case "s": // 访问次数数据模拟
    ...
    setChart() {
      ...
      let myChart = this.$echarts.init(document.getElementById("doubleArea"));
      myChart.clear();
      myChart.resize();
